...
use app\models\AelLiftPmSearch;
...
class PmController extends Controller
{
    ...
    {
        $searchModel = new AelLiftPmSearch();
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $maxData+=1;
            $qrcodeNumber =  "SPM00000000".$maxData; 
            $generateQRcode = QrCode::png($qrcodeNumber,'../assets/qrcode/'.$qrcodeNumber.'.png');
            $model->pm_qr_code = $qrcodeNumber;
            
            $checklistdata = implode(',',$_POST['AelLiftPm']['pm_service_checklist']);
            $model->pm_service_checklist = $checklistdata;
            $model->pm_schedule_start_datetime = date("Y-m-d H:i:s",strtotime($_POST['AelLiftPm']['pm_schedule_start_datetime']));
            $model->pm_schedule_end_datetime = date("Y-m-d H:i:s",strtotime($_POST['AelLiftPm']['pm_schedule_end_datetime']));
            
            if($model->save()) {
                if($model->pm_status == '2') {
                    ...
                    if(empty($historyData)) {
                        ...
                        $connection->createCommand()
                                    ->insert('ael_service_histories', [
                                                    'pm_e_call' => $model->id,
                                                    'qr_code' => $model->pm_qr_code,
                                                    'lift_name' => $model->lift_name,
                                                    'lift_name_chi' => $model->lift_name_chi,
                                                    'lift_id' => $model->lift_id,
                                                    'created_datetime' => date("Y-m-d H:i:s"),
                                                    'updated_datetime' => date("Y-m-d H:i:s"),
                                                    'registered_datetime' => $model->created_datetime,
                                                    'schedule_start_datetime' => $model->pm_schedule_start_datetime,
                                                    'schedule_end_datetime' => $model->pm_schedule_end_datetime,
                                                    'client_id' => $model->client_id,
                                                    'worker_1_id' => $model->worker_1_id,
                                                    'worker_2_id' => $model->worker_2_id,
                                                    'supervisor_id' => $model->supervisor_id,
                                                    'service_checklist'=>$model->pm_service_checklist,
                                                    'status' => $model->pm_status
                                        ])
                                    ->execute();
                    }    
                }
                return $this->redirect('pm/index');
            } else {
                echo "<pre>";
    ...
    public function actionUpdate($id)
    {
        ...
        $checklistdata = explode(',',$model->pm_service_checklist);
        $model->pm_service_checklist = $checklistdata;
    ...
    {
        if (($model = AelLiftPm::findOne($id)) !== null) {
            return $model;
        } else {
