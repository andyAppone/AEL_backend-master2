 ...
 */
class AelUser extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    {
        return [
            [['first_name','first_name_chi','last_name','last_name_chi','gender','username','password','email','user_mobile','address','address_chi','is_active','user_type'], 'required','on' => 'create'],
            [['first_name','first_name_chi','last_name','last_name_chi','gender','user_mobile','address','address_chi','is_active','user_type'], 'required','on' => 'update'],
            ['email', 'email'],
            ...
            ['supervisor_id', 'required', 'when' => function ($model) {
                return $model->user_type == '2';
            }, 'whenClient' => "function (attribute, value) {
                return $('#aeluser-user_type').val() == '2';
            }"],
            ['supervisor_id', 'required', 'when' => function ($model) {
                return $model->user_type == '3';
            }, 'whenClient' => "function (attribute, value) {
                return $('#aeluser-user_type').val() == '3';
            }"],        
        ];
    }
